...
protected:
	...
	void	_Reference( _TyGraphNodeBase * _pgnbCur ) noexcept(true)
	{
...
public:
	...
	explicit _graph_node_iterator_base_safe(	t_TyGraphNodeSafe * _pgnbCur ) noexcept(true)
	...
	explicit _graph_node_iterator_base_safe( const t_TySafeNodeIter & _r ) noexcept(true)
#else //_STLP_MEMBER_TEMPLATES
	explicit _graph_node_iterator_base_safe( const _TyThis & _r ) noexcept(true)
...
#ifdef _STLP_MEMBER_TEMPLATES
	template < class t_TySafeNodeIter >
	explicit _graph_node_iterator_base_safe( const t_TySafeNodeIter & _r, std::false_type ) noexcept(true)
#else //_STLP_MEMBER_TEMPLATES
	explicit _graph_node_iterator_base_safe( const _TyThis & _r, std::false_type ) noexcept(true)
#endif //_STLP_MEMBER_TEMPLATES
		:	_TyIterBase( 0 )
	{
		_init();
...
// Initialize with a reference and a different allocator:
	...
	explicit _graph_node_iterator_base_safe(	const t_TyNodeIter & _r ) noexcept(true)
#else //_STLP_MEMBER_TEMPLATES
	explicit _graph_node_iterator_base_safe(	const _TyThis & _r ) noexcept(true)
#endif //_STLP_MEMBER_TEMPLATES
		:	_TyIterBase( 0 )
	...
	~_graph_node_iterator_base_safe() noexcept(true)
	{
		_Dereference();
	}
	...
	template < class t_TyNodeIter >
	_TyThis &	operator = ( const t_TyNodeIter & _r ) noexcept(true)
#else //_STLP_MEMBER_TEMPLATES
	_TyThis &	operator = ( const _TyThis & _r ) noexcept(true)
#endif //_STLP_MEMBER_TEMPLATES
	{
	...
	void			SetPGNBCur( _TyGraphNodeBase * _pgnbCur ) noexcept(true)
	{	
		if ( PGNBCur() )
		{
			m_gclNode.remove_link();
...
#else //_STLP_MEMBER_TEMPLATES
	void			Set_Populated( const _TyThis & _r ) noexcept(true)
#endif //_STLP_MEMBER_TEMPLATES
	{
...
protected:
	...
	void	_PushConnection( _TyGraphLinkBase ** _ppglbCur ) noexcept(true)
	{
	...
	void	_Reference( _TyGraphLinkBase ** _ppglbCur ) noexcept(true)
	{
		Assert( !PPGLBCur() );
...
protected:
	...
	explicit _graph_link_pos_iterator_base_safe( _TyGraphLinkBase ** _ppglbCur ) noexcept(true)
		: _TyIterBase( 0 )
	{
		_init();
		if ( _ppglbCur )
...
// Init with an iterator that has an allocator:
	...
	explicit _graph_link_pos_iterator_base_safe( const t_TyLinkPosIter & _r ) noexcept(true)
#else //_STLP_MEMBER_TEMPLATES
	explicit _graph_link_pos_iterator_base_safe( const _TyThis & _r ) noexcept(true)
#endif //_STLP_MEMBER_TEMPLATES
		: _TyIterBase( 0 )
	{
		_init();
...
#ifdef _STLP_MEMBER_TEMPLATES
	template < class t_TyLinkPosIter >
	explicit _graph_link_pos_iterator_base_safe( const t_TyLinkPosIter & _r, std::false_type ) noexcept(true)
#else //_STLP_MEMBER_TEMPLATES
	explicit _graph_link_pos_iterator_base_safe( const _TyThis & _r, std::false_type ) noexcept(true)
#endif //_STLP_MEMBER_TEMPLATES
		: _TyIterBase( 0 )
	{
		_init();
...
// Accessors:
	void		SetPPGLBCur( _TyGraphLinkBase ** _ppglbCur ) noexcept(true)
	{
		if ( PPGLBCur() )
		{ 
			m_gclObj.remove_link();
	...
	void		SetPPGLBCur_Populated( _TyGraphLinkBase ** _ppglbCur ) noexcept(true)
	{	
		Assert( _ppglbCur );
		if ( PPGLBCur() )
		{
...
#warning Think this code needs updating.
...
#else //_STLP_MEMBER_TEMPLATES
	void	ExchangeSiblingParents( _TyThis const & _rSiblingParent ) const noexcept(true)
	{
		Assert( &_rSiblingParent != this );
		Assert( PGLBCur()->m_pgnbNodeChild == _rSiblingParent.PGLBCur()->m_pgnbNodeChild );
		_TyGraphLinkBase::ExchangeParents( PGLBCur(), _rSiblingParent.PGLBCur() );
	}
	void	ExchangeSiblingChildren( _TyThis const & _rSiblingChild ) const noexcept(true)
	{
	...
	void	ExchangeSiblingParents( _TyLinkIdentIter const & _rlpiSiblingParent ) const noexcept(true)
	{
		Assert( &_rSiblingParent != this );
		Assert( PGLBCur()->m_pgnbNodeChild == _rSiblingParent.PGLBCur()->m_pgnbNodeChild );
		_TyGraphLinkBase::ExchangeParents( PGLBCur(), _rSiblingParent.PGLBCur() );
	}
	void	ExchangeSiblingChildren( _TyLinkIdentIter const & _rlpiSiblingChild ) const noexcept(true)
	{
		Assert( &_rSiblingChild != this );
		Assert( PGLBCur()->m_pgnbNodeChild == _rSiblingChild.PGLBCur()->m_pgnbNodeChild );
		_TyGraphLinkBase::ExchangeChildren( PGLBCur(), _rSiblingChild.PGLBCur() );
...
protected:
	...
	void	_Reference( _TyGraphLinkBase * _pglbCur ) noexcept(true)
	{
		Assert( !PGLSCur() );
		m_pglbCur = _pglbCur;
...
public:
	...
	explicit _graph_link_ident_iterator_base_safe(	const _TyGraphLinkBase * _pglbCur ) noexcept(true)
		:	_TyIterBase( 0 )
	{
		_init();
		if ( _pglbCur )
...
// Init with iterator that has an allocator:
	...
	explicit _graph_link_ident_iterator_base_safe( const _TyLinkIdentIter & _r ) noexcept(true)
#else //_STLP_MEMBER_TEMPLATES
	explicit _graph_link_ident_iterator_base_safe( const _TyThis & _r ) noexcept(true)
#endif //_STLP_MEMBER_TEMPLATES
		: _TyIterBase( 0 )
	{ 
		_init();
...
#ifdef _STLP_MEMBER_TEMPLATES
	template < class _TyLinkIdentIter >
	explicit _graph_link_ident_iterator_base_safe( const _TyLinkIdentIter & _r, std::false_type ) noexcept(true)
#else //_STLP_MEMBER_TEMPLATES
	explicit _graph_link_ident_iterator_base_safe( const _TyThis & _r, std::false_type ) noexcept(true)
#endif //_STLP_MEMBER_TEMPLATES
		: _TyIterBase( 0 )
	{ 
		_init();
...
// Init with iterator and allocator:
	...
	void		SetPGLBCur( _TyGraphLinkBase * _pglbCur ) noexcept(true)
	{
		if ( PGLSCur() )
		{ 
			m_gclLink.remove_link();
...
protected:
	...
	void	_InitConnLink( t_TyPathNodeSafe * _ppns ) noexcept(true)
	{
	...
	void	_ReReferencePath( void * _pvReference ) noexcept(true)
	{
	...
	void	_DestroyPath( _TyPathNodeBase ** _pppnbStart ) noexcept(true)
	{
		_TyPathNodeBase ** _pppnbOldTail = m_pppnbTail;
	...
	// The graph node to which this iterator is connected is being deinitialized.
	// We don't need to unlink the connection link.
	void	_node_deinit( _TyGraphConnectionLink * _pgclNode ) noexcept(true)
	{
	...
	// The graph link to which this iterator is connected is being deinitialized.
	// We don't need to unlink the connection link.
	void	_link_deinit( _TyGraphConnectionLink * _pgclLink ) noexcept(true)
	{
...
public:
	...
	explicit _graph_path_iterator_base_safe( t_TyPathNodeSafeAllocator const & _allocPathNode ) noexcept(true)
...
// Construction with a path and a different allocator:
...
#ifdef _STLP_MEMBER_TEMPLATES
	template < class _TyPathIter >
	explicit _graph_path_iterator_base_safe( _TyPathIter const & _r, std::false_type ) noexcept(true)
#else //_STLP_MEMBER_TEMPLATES
	explicit _graph_path_iterator_base_safe( _TyThis const & _r, std::false_type ) noexcept(true)
#endif //_STLP_MEMBER_TEMPLATES
		: _TyBaseAllocPathNode( _r._GetPNAllocatorAsPassed() ),
	...
	~_graph_path_iterator_base_notsafe() noexcept(true)
	{
		_DestroyPath( &m_ppnbHead );
	}

// Allocator access:
	_TyPathNodeSafeAllocator const &	_GetPNSAllocator() const noexcept(true)
	{
		return _TyBaseAllocPathNode::get_allocator();
	}
	t_TyPathNodeSafeAllocator					_GetPNAllocatorAsPassed() const noexcept(true)
	{
	...
	bool operator == ( t_TyLinkIter const & _r ) const noexcept(true)
#else //_STLP_MEMBER_TEMPLATES
	bool operator == ( _TyThis const & _r ) const noexcept(true)
#endif //_STLP_MEMBER_TEMPLATES
	{
		return PGLBCur() == _r.PGLBCur();
	}
	...
	_TyThis & operator = ( _TyIter const & _r ) noexcept(true)
#else //_STLP_MEMBER_TEMPLATES
	_TyThis & operator = ( _TyThis const & _r ) noexcept(true)
#endif //_STLP_MEMBER_TEMPLATES
	{
	...
	template < class _TyIter >
	void	swap( _TyIter & _r ) noexcept(true)
	{
	...
	template < >
	void	swap( _TyThis & _r ) noexcept(true)
	{
